...
# --- helper to rebuild unveiled IMAGE cards from DB-stored prepared window ---
def compose_from_prepared(prepared_png: bytes, frame_key: str, *, unveiled: bool) -> Image.Image:
    ...
    out = Image.new("RGBA", frame.size, (0, 0, 0, 0))
    out.alpha_composite(prepared, (x + dx, y + dy))   # photo under frame
    out.alpha_composite(frame, (0, 0))                # frame on top
    return out
...
class UnveilDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if is_correct and won:
            ...
            await msg.edit(attachments=[], files=[file], view=view, embed=None)
